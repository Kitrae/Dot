                                      ...
                                      float maxZ)
{
    ...
    const float points[][3] = {
        ...
        {centerX, centerY, minZ}, {centerX, centerY, maxZ},
    };

    const float* viewProj = camera.GetViewProjectionMatrix();
    ...
    for (const auto& point : points)
    {
        ...
        const float clipX = viewProj[0] * x + viewProj[4] * y + viewProj[8] * z + viewProj[12];
        const float clipY = viewProj[1] * x + viewProj[5] * y + viewProj[9] * z + viewProj[13];
        const float clipZ = viewProj[2] * x + viewProj[6] * y + viewProj[10] * z + viewProj[14];
        const float clipW = viewProj[3] * x + viewProj[7] * y + viewProj[11] * z + viewProj[15];
        ...
        closestDepth = std::min(closestDepth, std::clamp(ndcZ, 0.0f, 1.0f));
    ...
    const float centerU = 0.5f * (screenMinX + screenMaxX);
    const float centerV = 0.5f * (screenMinY + screenMaxY);
    float maxDepthSample = 0.0f;
    maxDepthSample = std::max(maxDepthSample, ReadHZBDepthSample(m_Occlusion, mipLevel, screenMinX, screenMinY));
    maxDepthSample = std::max(maxDepthSample, ReadHZBDepthSample(m_Occlusion, mipLevel, screenMaxX, screenMinY));
    maxDepthSample = std::max(maxDepthSample, ReadHZBDepthSample(m_Occlusion, mipLevel, screenMinX, screenMaxY));
    maxDepthSample = std::max(maxDepthSample, ReadHZBDepthSample(m_Occlusion, mipLevel, screenMaxX, screenMaxY));
    maxDepthSample = std::max(maxDepthSample, ReadHZBDepthSample(m_Occlusion, mipLevel, centerU, centerV));

    return closestDepth > (maxDepthSample + 0.01f);
}
