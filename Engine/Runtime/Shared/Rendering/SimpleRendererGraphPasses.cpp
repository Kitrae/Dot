                       ...
                       const std::vector<MaterialData>& resolvedMaterials, int* renderCallCounter)
{
    ...
    for (std::size_t drawIndex = 0; drawIndex < queue.size();)
    {
        ...
        for (std::size_t i = drawIndex; i < groupEnd; ++i)
        {
            ...
            renderer.RenderMesh(passCamera, swapChain, queue[i].worldMatrix.Data(), *queue[i].mesh);
...
void SSAOGraphPassExecutor::Execute(SimpleRenderer& renderer, RHISwapChain* swapChain, const Camera& camera)
{
    ...
    constants.aoParams1[2] = 0.0f;
    constants.aoParams1[3] = 0.0f;
    constants.aoParams2[0] = 1.0f / aoWidth;
    constants.aoParams2[1] = 1.0f / aoHeight;
    constants.aoParams2[2] = aoWidth / static_cast<float>(GetStableAONoiseWidth());
    constants.aoParams2[3] = aoHeight / static_cast<float>(GetStableAONoiseHeight());
...
void SSAOBlurGraphPassExecutor::Execute(SimpleRenderer& renderer, RHISwapChain* swapChain)
{
    ...
    blurConstants.nearZ = renderer.m_SSAONearZ;
    blurConstants.farZ = renderer.m_SSAOFarZ;
...
{
    if (!renderer.m_SSAOSettings.enabled || !renderer.m_SSAOBlurredRT || !renderer.m_SSAOSRVHeap || !swapChain)
        return;
    ...
    struct
    {
        ...
        float depthThreshold;
        float pad;
    } blurConst = {{1.0f / std::max(renderer.m_SSAOBufferWidth, 1u), 1.0f / std::max(renderer.m_SSAOBufferHeight, 1u)},
                   renderer.m_SSAOSettings.blurDepthThreshold,
                   0.0f};
