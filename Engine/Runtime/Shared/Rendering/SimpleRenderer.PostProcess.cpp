...
namespace
{
struct StableAONoiseUploadState
{
    ComPtr<ID3D12Resource> buffer;
    UINT64 sizeInBytes = 0;
};

std::unordered_map<void*, StableAONoiseUploadState> s_StableAONoiseUploadBuffers;
} // namespace
...
bool SimpleRenderer::CreateSSAOResources(RHISwapChain* swapChain, uint32_t width, uint32_t height)
{
    ...
    const bool psoIncomplete = (!m_SSAOPSO || !m_SSAOBlurPSO || !m_SSAODebugPSO || !m_SSAOCompositePSO);
    bool selectedExternalShaders = m_SSAOUsingExternalShaders;
    ...
    if (shaderModeChanged || psoIncomplete)
    {
    ...
    // 2. Create Noise Texture (larger stable tiled rotation pattern)
    std::vector<float> ssaoNoise;
    const auto& stableNoise = GetStableAONoisePattern();
    ssaoNoise.reserve(GetStableAONoiseFloatCount());
    for (const auto& sample : stableNoise)
        ssaoNoise.insert(ssaoNoise.end(), sample.begin(), sample.end());

    D3D12_RESOURCE_DESC noiseDesc = {};
    noiseDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
    noiseDesc.Width = static_cast<UINT>(GetStableAONoiseWidth());
    noiseDesc.Height = static_cast<UINT>(GetStableAONoiseHeight());
    ...
    releaseObject(m_SSAONoiseTexture);
    ...
    UINT noiseRowCount = 0;
    UINT64 noiseRowSizeInBytes = 0;
    UINT64 noiseUploadBufferSize = 0;
    d3dDevice->GetCopyableFootprints(&noiseDesc, 0, 1, 0, &noiseFootprint, &noiseRowCount, &noiseRowSizeInBytes,
                                     &noiseUploadBufferSize);

    D3D12_HEAP_PROPERTIES noiseUploadHeap = {};
    noiseUploadHeap.Type = D3D12_HEAP_TYPE_UPLOAD;

    D3D12_RESOURCE_DESC uploadDesc = {};
    uploadDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
    uploadDesc.Width = noiseUploadBufferSize;
    uploadDesc.Height = 1;
    ...
    StableAONoiseUploadState& uploadState = s_StableAONoiseUploadBuffers[d3dDevice];
    if (!uploadState.buffer || uploadState.sizeInBytes < noiseUploadBufferSize)
    {
        hr = d3dDevice->CreateCommittedResource(&noiseUploadHeap, D3D12_HEAP_FLAG_NONE, &uploadDesc,
                                                D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
                                                IID_PPV_ARGS(&uploadState.buffer));
        if (FAILED(hr) || !uploadState.buffer)
            return false;
        uploadState.sizeInBytes = noiseUploadBufferSize;
    }

    void* mappedNoiseData = nullptr;
    hr = uploadState.buffer->Map(0, nullptr, &mappedNoiseData);
    if (FAILED(hr) || !mappedNoiseData)
        return false;

    auto* mappedBytes = static_cast<uint8_t*>(mappedNoiseData) + noiseFootprint.Offset;
    const uint8_t* sourceBytes = reinterpret_cast<const uint8_t*>(ssaoNoise.data());
    const size_t sourceRowBytes = static_cast<size_t>(noiseRowSizeInBytes);
    for (UINT row = 0; row < noiseRowCount; ++row)
    {
        std::memcpy(mappedBytes + static_cast<size_t>(row) * noiseFootprint.Footprint.RowPitch,
                    sourceBytes + static_cast<size_t>(row) * sourceRowBytes, sourceRowBytes);
    }
    uploadState.buffer->Unmap(0, nullptr);

    D3D12_TEXTURE_COPY_LOCATION noiseDst = {};
    noiseDst.pResource = reinterpret_cast<ID3D12Resource*>(m_SSAONoiseTexture);
    noiseDst.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
    noiseDst.SubresourceIndex = 0;

    D3D12_TEXTURE_COPY_LOCATION noiseSrc = {};
    noiseSrc.pResource = uploadState.buffer.Get();
    noiseSrc.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
    noiseSrc.PlacedFootprint = noiseFootprint;

    cmdList->CopyTextureRegion(&noiseDst, 0, 0, 0, &noiseSrc, nullptr);

    D3D12_RESOURCE_BARRIER noiseBarrier = {};
    noiseBarrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    noiseBarrier.Transition.pResource = reinterpret_cast<ID3D12Resource*>(m_SSAONoiseTexture);
    noiseBarrier.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
    noiseBarrier.Transition.StateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
    noiseBarrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
    cmdList->ResourceBarrier(1, &noiseBarrier);

    // 3. Generate Sample Kernel
    m_SSAOKernel.clear();
    const auto& stableKernel = GetStableAOKernel();
    m_SSAOKernel.reserve(GetStableAOKernelFloatCount());
    for (const auto& sample : stableKernel)
        m_SSAOKernel.insert(m_SSAOKernel.end(), sample.begin(), sample.end());
    ...
    if (!m_SSAOKernelBuffer)
    {
        ...
        kernelDesc.Width = static_cast<UINT64>(GetStableAOKernelByteCount());
        kernelDesc.Height = 1;
        ...
        D3D12_HEAP_PROPERTIES kernelUploadHeap = {};
        kernelUploadHeap.Type = D3D12_HEAP_TYPE_UPLOAD;

        d3dDevice->CreateCommittedResource(&kernelUploadHeap, D3D12_HEAP_FLAG_NONE, &kernelDesc,
                                           D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
    ...
    kernelSrvDesc.Buffer.NumElements = static_cast<UINT>(GetStableAOKernelSampleCount());
    ...
    // 7. Create SSAO PSOs (also recreate if any of the AO PSOs are missing)
    if (!m_SSAOPSO || !m_SSAODebugPSO || !m_SSAOBlurPSO || !m_SSAOCompositePSO)
    {
        ...
        // Cheap stable fixed-pattern SSAO.
        std::string ssaoPS = R"(
            ...
            float3 ReconstructNormal(float2 uv, float2 texelSize, float nearZ, float farZ) {
                ...
                float d1 = LinearizeDepth(DepthTex.Sample(PointClamp, uv + float2(texelSize.x, 0)).r, nearZ, farZ);
                float d2 = LinearizeDepth(DepthTex.Sample(PointClamp, uv + float2(0, texelSize.y)).r, nearZ, farZ);
        ...
        // Wider depth-aware bilateral blur shader
        ...
        const char* ssaoDebugPS = R"(
            Texture2D AOBlurred : register(t3);
            ...
            float4 main(PS_INPUT input) : SV_TARGET {
                float ao = AOBlurred.Sample(LinearClamp, input.uv).r;
                return float4(ao, ao, ao, 1.0);
            }
        )";
        ...
        ComPtr<ID3DBlob> ssaoVsBlob, ssaoPsBlob, blurPsBlob, debugPsBlob, errorBlob;
        bool useFileShaders = preferExternalShaders && (vsShader && ssaoShader && blurShader);
        selectedExternalShaders = useFileShaders;
        ...
        if (!useFileShaders)
        {
            ...
            D3DCompile(ssaoPS.c_str(), ssaoPS.size(), "SSAOPS", nullptr, nullptr, "main", "ps_5_0", 0, 0,
                       &ssaoPsBlob, &errorBlob);
        ...
        D3DCompile(ssaoDebugPS, strlen(ssaoDebugPS), "SSAODebugPS", nullptr, nullptr, "main", "ps_5_0", 0, 0,
                   &debugPsBlob, &errorBlob);

        bool hasShaders = (useFileShaders || (ssaoVsBlob && ssaoPsBlob && blurPsBlob)) && debugPsBlob;
        if (hasShaders && m_SSAORootSignature)
        {
            ...
            if (FAILED(hr) || !m_SSAOCompositePSO)
                return false;
        }
    }

    m_SSAOUsingExternalShaders = selectedExternalShaders;
    RefreshGraphInteropTextures();
